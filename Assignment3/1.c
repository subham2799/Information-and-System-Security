...
char* encrypt(char plaintext[MAX_LIMIT], int key, char alphabets[26]){
    ...
    for(int i = 0; i < size; i++)
    {
        ...
        if(ptr != NULL){
            //Autokey cipher encryption algorithm

            //get index of current ciphertext character from alphabet set
            int num = (ptr - alphabets);

            int ciphernum = (num + key) % alphabetsize;
            ciphertext[i] = alphabets[ciphernum];
            //making current plaintext, the key for next encryption
            key = num;
        }
        else
...
char* decrypt(char ciphertext[MAX_LIMIT], int key, char alphabets[26]){
    ...
    for(int i = 0; i < size; i++)
    {
        ...
        if(ptr != NULL){
            //Autokey cipher decryption algorithm
            ...
            //making current deciphered plaintext, the key for next decryption
            key = num;
        }
        else
